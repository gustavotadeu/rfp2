...
const RFPListPage = () => {
  ...
  const fetchRFPs = async () => {
    ...
    try {
      ...
      const vendorMap = new Map(
        vendorsRes.data.map((v: any) => [v.id, v.nome])
      );
      const enriched = rfpsRes.data.map((rfp: RFP) => ({
        ...rfp,
        fabricante_escolhido_nome: rfp.fabricante_escolhido_id
          ? vendorMap.get(rfp.fabricante_escolhido_id) || ""
          : "",
      }));
      setRfps(enriched);
    } catch {
  ...
  const handleDownload = async (rfpId: number, filename: string) => {
    try {
      const response = await api.get(`/rfps/${rfpId}/download`, {
        responseType: "blob",
      });
  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-2">
        <h2 className="text-3xl font-bold bg-blue-900 bg-clip-text text-transparent drop-shadow">
          RFPs
        </h2>
        <button
          onClick={() => setModalOpen(true)}
          className="bg-blue-900 text-white px-6 py-2 rounded-xl font-bold shadow hover:scale-105 active:scale-95 transition-all text-lg"
        >
          + Nova RFP
        </button>
      </div>
      <div className="bg-white/90 rounded-2xl shadow-xl p-4 md:p-8">
        <div className="mb-6 flex flex-col sm:flex-row gap-2 items-center">
          <input
            type="text"
            placeholder="Buscar..."
            className="border px-4 py-2 rounded-lg w-full sm:w-64 focus:ring-2 focus:ring-primary"
          />
          <button className="bg-blue-900 text-white px-5 py-2 rounded-lg font-semibold shadow hover:scale-105 transition">
            Filtrar
          </button>
        </div>
        {loading ? (
            ...
            <table className="min-w-full table-auto text-base">
              <thead>
                <tr className="bg-blue-50">
                  <th className="px-6 py-3 text-left font-bold text-blue-800">
                    Nome
                  </th>
                  <th className="px-6 py-3 text-left font-bold text-blue-800">
                    Status
              ...
              <tbody>
                {rfps.map((rfp) => (
                  <tr
                    key={rfp.id}
                    className="border-b border-slate-100 hover:bg-primary/5 transition"
                  >
                    ...
                    <td className="px-6 py-3 flex gap-2">
                      <button
                        ...
                        className="bg-blue-900 text-white px-4 py-1 rounded-lg font-semibold shadow hover:bg-primary/80 transition"
                      >
                        Ver
                      </button>
                      <button
                        onClick={() => handleDelete(rfp.id)}
                        className="bg-red-500 text-white px-4 py-1 rounded-lg font-semibold shadow hover:bg-red-600 transition"
                      >
                        Excluir
                      </button>
                    </td>
