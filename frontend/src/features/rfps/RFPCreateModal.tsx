...
const RFPCreateModal: React.FC<RFPCreateModalProps> = ({
  open,
  onClose,
  onCreated,
}) => {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (!rfpId || rfpId === "undefined") {
        setError(
          "Erro ao obter o ID da RFP criada. Não é possível enviar o arquivo."
        );
        setUploadProgress(null);
        return;
      }
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black/40 z-50">
      <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md relative animate-fadeIn">
        ...
        <h3 className="text-2xl font-bold mb-6 text-blue-900">Nova RFP</h3>
        <form
        ...
        >
          <div>
            ...
            <input
              {...register("nome")}
              className="w-full border rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-900"
            />
            {errors.nome && (
          ...
          <div>
            ...
            <input
              ...
              {...register("arquivo")}
              className="w-full border rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-900 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:bg-blue-900/90 file:text-white file:font-semibold hover:file:bg-blue-950"
            />
            {errors.arquivo && (
              <span className="text-red-500 text-sm">
          ...
          <button
            ...
            className="w-full bg-blue-900 text-white py-2 rounded-xl font-bold shadow hover:bg-blue-950 active:scale-95 transition-all mt-2 disabled:opacity-60"
          >
            {uploadProgress !== null
